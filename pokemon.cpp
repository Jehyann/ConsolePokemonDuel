...
Pokemon::Pokemon(const std::string& _nom, int _niveau, const std::string& _type)
    : nom(_nom), niveau(_niveau), type(_type) {}
...
void Pokemon::afficherInfo() const {
    ...
    for (const Attaque& attaque : attaques) {
        std::cout << "- " << attaque.nom << " (" << attaque.description << ", Dégâts : " << attaque.degats << ")" << std::endl;
    }
}